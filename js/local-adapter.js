/* Expedition Board — local adapter.
   ...
   Implements the same contract as the Firebase adapter so the UI can't tell them apart:
     ...
     postSession(session) -> id
     seat(sessId, character)  unseat(sessId, charId)  move(fromId, toId, charId)   (throw Error(reason) to refuse)
     log(kind, text, meta)      writes one dispatch
     reset()                    local only

   state = { me, players[], sessions[], dispatches[] } — see data.js for the shapes. */
window.KS = window.KS || {};
(function (KS) {
  'use strict';
  const KEY = 'kresthalis-expedition-board:v2';

  class LocalAdapter {
    constructor() { this.state = null; this.onState = null; }
    ...
    async postSession(s) {
      const id = 's' + KS.util.uid();
      this.state.sessions.push(Object.assign({}, s, { id, party: [], locked: false, gmUid: this.state.me.uid, postedAt: Date.now() }));
      this._emit();
      return id;
    }
